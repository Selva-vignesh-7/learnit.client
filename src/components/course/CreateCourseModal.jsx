...
function CreateCourseModal({ onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const isExternalCourse = sourceUrl && 
      sourceUrl.length > 5 &&
      !sourceUrl.includes("youtube.com") && 
      !sourceUrl.includes("youtu.be");
    ...
    if (isExternalCourse) {
      ...
      if (!formData.totalEstimatedHours?.trim() || parseFloat(formData.totalEstimatedHours) <= 0) {
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      if (isYouTube) {
        setAiProgress("Extracting YouTube video metadata and creating course structure...");
      } else {
        setAiProgress("Creating course structure with AI...");
      }
      
      const draft = await aiApi.createCourse(payload);
      console.log(isYouTube ? "[YouTube create-course draft]" : "[AI create-course draft]", draft);
      
      if (!draft || !draft.title) {
        throw new Error("AI didn't generate a valid course structure. Please try again with more details.");
      }
      ...
      const safeModules = (draft.modules || []).map((m, idx) => {
        ...
        const hours = Number.parseFloat((m.estimatedHours ?? m.EstimatedHours) || 0);
        ...
        const subModules = subModulesList.map((s, subIdx) => {
          const sh = Number.parseFloat((s.estimatedHours || s.EstimatedHours) || 0);
          const subDuration = Number.isFinite(sh) && sh > 0 ? sh : 1;
          return {
            id: crypto.randomUUID(),
            title: cleanText(s.title || s.Title)?.trim() || `Lesson ${subIdx + 1}`,
            duration: toHoursString(subDuration, "1"),
          };
        });

        const moduleTitle = cleanText(m.title || m.Title)?.trim() || `Module ${idx + 1}`;
        console.log(`[Frontend] Processing module ${idx + 1}: "${moduleTitle}" with ${subModules.length} submodules${notes ? ` (has Notes: ${notes.substring(0, 50)}...)` : ' (no Notes)'}`);
      ...
      console.log("[Frontend] Processed modules:", safeModules.length, safeModules);
      ...
      if (isExternalCourse) {
        console.log("[Course Creation] External course detected - Quiz system will be enabled");
      }
    ...
    >
        ...
        <div className={styles.aiPane}>
          <div style={{ marginBottom: "1rem" }}>
            <p style={{ color: "#666", fontSize: "0.9rem", marginBottom: "0.5rem" }}>
              Tell AI what you want to learn, and we'll create a comprehensive course structure with modules, submodules, and learning objectives.
            </p>
            <div style={{ 
              padding: "0.75rem", 
              background: "#f0f9ff", 
              ...
              <strong>💡 Tips:</strong> Be specific about your learning goals, duration, and difficulty level for better results.
            </div>
          </div>
          ...
          <Field label="What do you want to learn? *">
            ...
            <div style={{ marginTop: "0.5rem", fontSize: "0.85rem", color: "#666" }}>
              ...
              <ul style={{ margin: "0.25rem 0 0 1.25rem", padding: 0 }}>
                <li>"Master ASP.NET Core MVC in 6 weeks - from basics to building REST APIs"</li>
                <li>"Learn Docker and Kubernetes for DevOps - hands-on containerization course"</li>
                <li>"Python data science course: pandas, numpy, matplotlib, and machine learning basics"</li>
              </ul>
            </div>
          </Field>
          ...
          <div className={ui.formGrid}>
            <Field label="Subject area (optional)">
              <select
                ...
                onChange={(e) => {
                  setFormData((prev) => ({ ...prev, subjectArea: e.target.value }));
                }}
          ...
          <div className={ui.formGrid}>
            <Field label="Estimated duration (optional)">
              <input
                ...
                value={formData.totalEstimatedHours ? Math.ceil(parseFloat(formData.totalEstimatedHours) / 10) : ""}
                onChange={(e) => {
                  const weeks = parseInt(e.target.value) || 0;
                  if (weeks > 0) {
                    setFormData((prev) => ({ ...prev, totalEstimatedHours: (weeks * 10).toString() }));
                  } else {
                    setFormData((prev) => ({ ...prev, totalEstimatedHours: "" }));
                  }
                }}
                style={{ padding: "0.5rem" }}
              />
          ...
          <Field label="Learning goals (optional)">
            ...
            />
            <small style={{ color: "#666", fontSize: "0.85rem", display: "block", marginTop: "0.25rem" }}>
              Specific outcomes you want to achieve (one per line or separated by commas)
            </small>
          </Field>
      ...
      {activeTab === "source" && (
        <div className={styles.aiPane}>
          <div style={{ marginBottom: "1rem" }}>
            <p style={{ color: "#666", fontSize: "0.9rem", marginBottom: "0.5rem" }}>
              Enter a URL from YouTube, educational websites, or course pages. We'll analyze the content and generate a structured course.
            </p>
            <div style={{ 
              padding: "0.5rem", 
              background: "#fef3c7", 
              ...
              <strong>Supported:</strong> YouTube videos/playlists, Medium articles, Dev.to posts, GitHub repos, documentation sites
            </div>
          </div>
          ...
          <Field label="Course Source URL *">
            ...
            {formData.sourceUrl && (
              <div style={{ marginTop: "0.5rem", fontSize: "0.85rem", color: "#666" }}>
                {formData.sourceUrl.includes("youtube.com") || formData.sourceUrl.includes("youtu.be") ? (
                  <span>📹 YouTube video detected - Will extract title, duration, and chapters</span>
                ) : formData.sourceUrl.includes("medium.com") || formData.sourceUrl.includes("towardsdatascience.com") || formData.sourceUrl.includes("freecodecamp.org") ? (
                  <span>📝 Medium article detected - Will extract title, reading time, and sections</span>
                ) : formData.sourceUrl.includes("dev.to") ? (
                  <span>📝 Dev.to article detected - Will extract title and reading time</span>
                ) : formData.sourceUrl.includes("github.com") ? (
                  <span>💻 GitHub repository detected - Will parse README and structure</span>
                ) : formData.sourceUrl.includes("docs.") || formData.sourceUrl.includes("documentation") ? (
                  <span>📚 Documentation site detected - Will extract table of contents</span>
                ) : (
                  <span>🌐 External course detected - Quiz system will be enabled for module completion</span>
                )}
                {formData.sourceUrl && 
                 ...
                 !formData.sourceUrl.includes("youtu.be") && (
                  <div style={{ 
                    ...
                    <strong>ℹ️ Quiz Required:</strong> Modules in this course will require passing a quiz before marking as complete.
                  </div>
                )}
              </div>
            )}
          ...
          {urlAnalysis && (
            <div style={{ 
              ...
              <div style={{ fontWeight: "600", marginBottom: "0.5rem", color: "#0369a1", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                <span>✓</span>
                <span>Analysis Complete - Course Generated!</span>
              </div>
              <div style={{ fontSize: "0.9rem", color: "#075985", marginBottom: "0.5rem" }}>
                <div><strong>Title:</strong> {urlAnalysis.title || "Course"}</div>
                {urlAnalysis.moduleCount > 0 && (
                  <div style={{ marginTop: "0.25rem" }}>
                    <strong>Structure:</strong> {urlAnalysis.moduleCount} modules • ~{urlAnalysis.totalHours} hours estimated
                  </div>
                )}
              </div>
              <div style={{ fontSize: "0.85rem", color: "#0284c7", fontStyle: "italic" }}>
                Review and adjust the course details in the "Manual" tab, then save.
              </div>
            </div>
          )}
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          {(() => {
            ...
            return isExternalCourse ? (
              <div style={{ 
                ...
                <strong>⚠️ External Course Requirements:</strong> All course details and module information must be completed before saving.
              </div>
            ) : null;
          })()}
          <div className={ui.formGrid}>
          ...
          <Field label="Course Source URL">
            ...
            {formData.sourceUrl && (
              <div style={{ marginTop: "0.5rem", fontSize: "0.85rem" }}>
                {formData.sourceUrl.includes("youtube.com") || formData.sourceUrl.includes("youtu.be") ? (
                  <span style={{ color: "#666" }}>📹 YouTube course - Video playback enabled</span>
                  ...
                  <div style={{ 
                    ...
                    <strong>ℹ️ External Course Detected:</strong> Quiz system will be enabled. Modules will require passing a quiz before marking as complete.
                  </div>
                ) : null}
              </div>
            )}
          ...
          <Field label={(() => {
            ...
            const isExternalCourse = sourceUrl && 
              ...
              !sourceUrl.includes("youtube.com") && 
              !sourceUrl.includes("youtu.be");
            return isExternalCourse ? "Learning objectives *" : "Learning objectives";
          })()}>
            <textarea
          ...
          <div className={ui.formGrid}>
            <Field label={(() => {
              ...
              const isExternalCourse = sourceUrl && 
                ...
                !sourceUrl.includes("youtube.com") && 
                !sourceUrl.includes("youtu.be");
              return isExternalCourse ? "Difficulty level *" : "Difficulty level";
            })()}>
              <select
                name="difficulty"
                value={formData.difficulty}
