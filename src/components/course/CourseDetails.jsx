...
import { courseApi, scheduleApi, aiApi, quizApi, classroomApi } from "../../services";
import { parseYouTubeChapters, mapChaptersToSubModules } from "../../utils/youtubeChapters";
...
function CourseDetails() {
  ...
  const [manualProgress, setManualProgress] = useState({ hours: 0, minutes: 0 });
  ...
  const lastCompletedModuleRef = useRef({ moduleId: null, startTimeSeconds: 0, moduleIndex: -1 });
  ...
  const fetchCourse = useCallback(
    ...
    try {
      ...
      if (data?.modules) {
        ...
        console.log(`[Progress] Reset tracking for ${data.modules.length} modules`);
        console.log(`[Manual Override] Currently tracking ${manuallyUncheckedModulesRef.current.size} manually unchecked modules`);
      }
    } catch (err) {
    ...
    const checkExternalViewing = () => {
      try {
        ...
        if (data) {
          ...
          if (timeSinceOpen > 30000 && timeSinceOpen < 3600000) { // Between 30s and 1 hour
  ...
  const maybeAdjustSchedule = useCallback(
    async (reason, currentSeconds, durationSeconds) => {
      ...
      const finishedClip =
        durationSeconds && percentFromTimes(currentSeconds, durationSeconds) >= 90;
      const behindSchedule = remaining > scheduled + 1;
      
      // Also trigger on module completion events
      const moduleCompleted = reason.includes("module_completed") || reason.includes("playlist");
      ...
      try {
        ...
        console.log(`[Schedule] Auto-adjusted schedule for course ${course.id} (${watchedHours.toFixed(2)}h watched, reason: ${reason})`);
  ...
  const mapChaptersToSubModules = useCallback((chapters, modules) => {
    ...
    return chapters.map((chapter, chapterIdx) => {
      ...
      const byIndex =
        chapterIdx < allSubModules.length
          ? allSubModules[chapterIdx]
          : null;
   ...
   */
  const debouncedBackendSync = useCallback(async (courseId, currentTime, reason) => {
    ...
    if (reason === "play" || reason === "paused" || reason === "ended" || reason === "seeked") {
      try {
        const hours = Math.max(0, currentTime / 3600);
        const res = await courseApi.updateCourseActiveTime(courseId, Number(hours.toFixed(3)));
    ...
    if (timeSinceLastSync < 5000) {
      ...
      backendSyncTimeoutRef.current = setTimeout(async () => {
        try {
          const hours = Math.max(0, currentTime / 3600);
          await courseApi.updateCourseActiveTime(courseId, Number(hours.toFixed(3)));
          lastBackendSyncRef.current = Date.now();
        } catch (err) {
  ...
  const syncPlayback = useCallback(
    async ({ currentTime, duration, percent, playlistVideoIndex, playlistVideoId, videoEnded }, reason = "interval") => {
      if (!course?.id || !Number.isFinite(currentTime) || currentTime < 0) return;
      ...
      if (currentTime >= MIN_SAVE_TIME_LOCAL || (reason === "ended" && currentTime > 0)) {
        const storageKey = `learnit_playback_${course.id}_${playback.videoId || playlistVideoId || playback.playlistId || 'default'}`;
        try {
      ...
      if (playback.kind === "youtube" && course?.id) {
        ...
        const shouldSave = currentTime >= MIN_SAVE_TIME || (reason === "ended" && currentTime > 0);
          ...
          try {
            ...
            for (const module of modules) {
              try {
                ...
                if (notes) {
                  ...
                  if (metadata.startTimeSeconds !== undefined && currentTime >= metadata.startTimeSeconds) {
                    if (!currentModule || (metadata.startTimeSeconds > (JSON.parse(currentModule.notes || currentModule.Notes || "{}").startTimeSeconds || 0))) {
            ...
            if (playback.playlistId && saveVideoId) {
              try {
                ...
                localStorage.setItem(lastVideoKey, JSON.stringify({
                  videoId: saveVideoId,
                  timestamp: Date.now()
                }));
              } catch (e) {
            ...
            console.log(`[Playback Position] 💾 Saved current position: ${currentTime.toFixed(1)}s / ${duration?.toFixed(1) || 0}s (videoId: ${saveVideoId || 'NONE'}, moduleId: ${currentModule?.id || 'null'})`);
          } catch (err) {
        ...
        // Debounced sync for timeupdate events (every 5 seconds max)
          ...
          try {
            ...
            lastPlaybackPositionSyncRef.current = now;
            console.log(`[Playback Position] 💾 Saved current position (debounced): ${currentTime.toFixed(1)}s (videoId: ${saveVideoId || 'NONE'})`);
          } catch (err) {
            console.warn("Failed to save playback position to database (debounced):", err);
          }
        }
        else if (reason === "timeupdate" && timeSinceLastSync < 5000) {
          // Schedule sync for later
          ...
          playbackPositionSyncTimeoutRef.current = setTimeout(async () => {
            try {
              ...
              console.warn("Failed to save playback position to database (scheduled):", err);
            }
          }, 5000 - timeSinceLastSync);
        }
      }
      ...
      const shouldProcess = reason === "play" || reason === "paused" || reason === "ended" || reason === "seeked" || reason === "timeupdate";
      ...
      if (playback.kind === "youtube" && currentTime < 3 && reason !== "seeked" && reason === "timeupdate") {
        // Only skip if we're actually resuming OR if we're at the very beginning (< 2s)
        if (isResumingRef.current || currentTime < 2) {
          if (Math.random() < 0.2) {
            console.log(`[Progress] Skipping module check - ${isResumingRef.current ? 'resuming' : 'just started'} (currentTime: ${currentTime.toFixed(1)}s)...`);
          }
          return;
        }
      }
      ...
      try {
        ...
        if (!modules.length || !duration || duration <= 0) {
          if (reason === "timeupdate" && Math.random() < 0.1) { // Log 10% of timeupdate events for debugging
            console.log(`[Progress] Skipping - modules: ${modules.length}, duration: ${duration}`);
          }
          return;
        }
        
        // Debug: Log periodically that we're processing progress
        if (reason === "timeupdate" && Math.random() < 0.05) { // Log 5% of timeupdate events
          console.log(`[Progress] Processing progress check - time: ${currentTime.toFixed(1)}s, duration: ${duration.toFixed(1)}s, modules: ${modules.length}`);
        }
        ...
        const isYouTubeVideo = playback.kind === "youtube" && !playback.playlistId;
        ...
        if (isYouTubeVideo) {
          ...
          if (reason === "timeupdate" && Math.random() < 0.01) {
            console.log(`[Progress] Building module time map for ${modules.length} modules (YouTube video, duration: ${duration.toFixed(1)}s = ${(duration/60).toFixed(1)} minutes)`);
            console.log(`[Progress] Module names: ${modules.map((m, idx) => `${idx + 1}. ${m.title}`).join(', ')}`);
          }
          ...
          if (moduleTimings.length > 0) {
            ...
            moduleTimings.forEach(mt => {
              console.log(`  Module ${mt.index + 1} '${mt.title}': startTime=${mt.startTimeSeconds ?? 'N/A'}s (${mt.startTimeSeconds ? (mt.startTimeSeconds/60).toFixed(1) + 'min' : 'N/A'}), duration=${mt.durationSeconds ?? 'N/A'}s`);
            });
          }
          ...
          for (let i = 0; i < modules.length; i++) {
            ...
            try {
              ...
              if (notes) {
                ...
                if (metadata.startTimeSeconds !== undefined && metadata.startTimeSeconds !== null) {
                ...
                if (i + 1 < modules.length) {
                  const nextNotes = modules[i + 1].notes || modules[i + 1].Notes || "";
                  if (nextNotes) {
                    try {
                      const nextMetadata = JSON.parse(nextNotes);
                      if (nextMetadata.startTimeSeconds !== undefined && nextMetadata.startTimeSeconds !== null) {
                ...
                if (calculatedEndTime === null && metadata.durationSeconds !== undefined && metadata.durationSeconds !== null && metadata.durationSeconds > 0) {
                ...
                if (hasNotesData) {
                  console.log(`[Progress] Module ${i + 1}/${modules.length} '${module.title}': startTime=${startTime.toFixed(1)}s (${(startTime/60).toFixed(1)}min), endTime=${endTime.toFixed(1)}s (${(endTime/60).toFixed(1)}min), duration=${moduleDuration.toFixed(1)}s (${(moduleDuration/60).toFixed(1)}min) - ALIGNED with video section`);
                } else {
                  console.warn(`[Progress] Module ${i + 1} '${module.title}': NO Notes data - using fallback timing`);
                }
              }
            } catch (e) {
              console.warn(`[Progress] Failed to parse Notes for module ${i} '${module.title}':`, e);
            ...
            if (!hasNotesData) {
              ...
              if (estimatedHours > 0) {
                ...
                moduleDuration = remainingModules > 0 ? remainingTime / remainingModules : 0;
                endTime = startTime + moduleDuration;
              }
              ...
              if (i < 3) {
                console.log(`[Progress] Module ${i} '${module.title}': Using fallback timing (estimatedHours: ${estimatedHours}h) - startTime=${startTime.toFixed(1)}s, duration=${moduleDuration.toFixed(1)}s, endTime=${endTime.toFixed(1)}s`);
              }
            }
          ...
          if (moduleTimeMap.length > 0 && reason === "timeupdate" && Math.random() < 0.01) {
            console.log(`[Progress] Final module time map (${moduleTimeMap.length} modules):`);
            moduleTimeMap.forEach((item, idx) => {
              console.log(`  ${idx + 1}. '${item.module.title}': [${item.startTime.toFixed(1)}s - ${item.endTime.toFixed(1)}s] (${(item.duration/60).toFixed(1)}min)`);
            });
          }
        } else {
          ...
          if (playback.kind === "youtube" && sourceLink && sourceLink.url) {
            const description = course.description || sourceLink.description || "";
          ...
          for (const module of modules) {
            ...
            const moduleStartTime = subModuleTimeMap.length > 0 
            ...
            if (chapters.length > 0 && subModules.length > 0) {
              ...
              const mappedChapters = mapChaptersToSubModules(chapters, modules, duration);
              
              for (let i = 0; i < subModules.length; i++) {
                const subModule = subModules[i];
                const mapped = mappedChapters.find(m => m.subModule.id === subModule.id);
                
                if (mapped) {
                  // Use chapter timestamp as start time
                  const startTime = mapped.startTime;
                  const endTime = mapped.endTime !== null ? mapped.endTime : duration;
              ...
              for (const subModule of subModules) {
                ...
                const subModuleDuration = estimatedHours > 0 
                  ? estimatedHours * 3600 
                  : (duration - moduleEndTime) / (subModules.length - subModuleTimeMap.filter(sm => sm.module.id === module.id).length);
        ...
        for (let i = 0; i < moduleTimeMap.length; i++) {
          ...
          if (currentTime >= item.startTime && (i === moduleTimeMap.length - 1 || currentTime < item.endTime)) {
        ...
        for (let i = 0; i < subModuleTimeMap.length; i++) {
          ...
          if (currentTime >= item.startTime && (i === subModuleTimeMap.length - 1 || currentTime < item.endTime)) {
        ...
        if (now - lastProgressLogRef.current > 10000) {
          if (currentModule && currentModuleIndex >= 0) {
            ...
            if (moduleItem) {
              const moduleProgress = moduleItem.duration > 0 
                ? ((currentTime - moduleItem.startTime) / moduleItem.duration) * 100 
                : 0;
              console.log(`[Progress] Current module: '${currentModule.title}' (${currentModuleIndex + 1}/${moduleTimeMap.length}), completed: ${currentModule.isCompleted}, progress: ${moduleProgress.toFixed(1)}%, time: ${currentTime.toFixed(1)}s/${moduleItem.endTime.toFixed(1)}s`);
            }
          }
          if (currentSubModule) {
            console.log(`[Progress] Current submodule: ${currentSubModule.title} (index ${currentSubModuleIndex}), completed: ${currentSubModule.isCompleted}`);
          }
        ...
        const isPlaylist = playbackForSeek.playlistId || playlistVideoId || moduleTimeMap.length === 0;
        if (reason === "seeked" && playbackForSeek.kind === "youtube" && !isPlaylist && moduleTimeMap.length > 0) {
          ...
          if (currentSubModuleIndex >= 0) {
            for (let i = 0; i <= currentSubModuleIndex; i++) {
              const item = subModuleTimeMap[i];
              if (!item.subModule.isCompleted && currentTime >= item.startTime) {
          ...
          for (const subModuleId of completedSubModuleIds) {
            const item = subModuleTimeMap.find(sm => sm.subModule.id === subModuleId);
            if (item) {
              try {
                const subModules = (item.module.subModules || []).sort((a, b) => (a.order || 0) - (b.order || 0));
                const subModuleIndex = subModules.findIndex(sm => sm.id === subModuleId);
                if (subModuleIndex >= 0) {
                  await courseApi.updateChapterProgress(course.id, item.subModule.title, subModuleIndex, 100);
                  console.log(`[Seek] SubModule '${item.subModule.title}' marked complete`);
                }
              } catch (err) {
                ...
                if (subModules.length > 0) {
                  await Promise.all(
                    subModules.map((sm, idx) =>
                      courseApi.updateChapterProgress(course.id, sm.title, idx, 100).catch(() => null)
                    )
                  );
                }
        ...
        if (playback.kind === "youtube" && !playback.playlistId && moduleTimeMap.length > 0) {
          const completionThreshold = 0.90; // Mark complete at 90% of module duration
          ...
          for (let i = 0; i < moduleTimeMap.length; i++) {
            ...
            if (moduleItem.startTime < 0 || moduleItem.endTime <= moduleItem.startTime) {
              continue;
            }
            ...
            const thresholdTime = moduleItem.startTime + (moduleDuration * completionThreshold);
            const moduleProgress = ((currentTime - moduleItem.startTime) / moduleDuration) * 100;
            
            // Mark complete if we've passed the threshold OR passed the end time
            const shouldBeCompleted = currentTime >= thresholdTime || currentTime >= moduleItem.endTime;
            ...
            if (shouldBeCompleted !== currentlyCompleted) {
              ...
              if (shouldBeCompleted && manuallyUncheckedModulesRef.current.has(module.id)) {
                continue;
              }
              
              // Skip if we just processed this module (prevent rapid toggling)
              const lastProcessed = completedModulesThisSessionRef.current.get(module.id);
              const timeSinceLastProcess = lastProcessed ? Date.now() - lastProcessed : Infinity;
              ...
              try {
                console.log(`[Progress] ${shouldBeCompleted ? '✅' : '❌'} Module '${module.title}' ${shouldBeCompleted ? 'completed' : 'incomplete'} (time: ${currentTime.toFixed(1)}s ${shouldBeCompleted ? '>=' : '<'} ${thresholdTime.toFixed(1)}s, progress: ${moduleProgress.toFixed(1)}%)`);
                
                // Use set-completion endpoint for auto-tracking (ensures persistence)
                await courseApi.setModuleCompletion(module.id, shouldBeCompleted);
                ...
                setCourse((prev) => {
                  ...
                  const updatedModules = prev.modules.map(m =>
                    m.id === module.id ? { ...m, isCompleted: shouldBeCompleted } : m
                  );
                ...
                if (shouldBeCompleted) {
                  try {
                    await maybeAdjustSchedule("module_completed", currentTime, duration);
                  } catch (scheduleErr) {
                ...
                console.error(`❌ Failed to update module '${module.title}':`, err);
              }
            }
          }
        }
        ...
        if (playback.kind === "youtube" && playback.playlistId) {
          ...
          const videoEndedOrComplete = videoEnded || (duration > 0 && percent >= 80);
          
          // Debug logging for playlist completion
          if (videoEndedOrComplete || (duration > 0 && percent >= 75)) {
            console.log(`[Playlist Progress] Checking completion - videoEnded: ${videoEnded}, percent: ${percent.toFixed(1)}%, videoId: ${currentPlaylistVideoId || 'NONE'}, index: ${currentPlaylistVideoIndex !== undefined ? currentPlaylistVideoIndex : 'NONE'}`);
          }
          ...
          if (videoEndedOrComplete && (currentPlaylistVideoId || (currentPlaylistVideoIndex !== undefined && currentPlaylistVideoIndex >= 0))) {
            const statusText = videoEnded ? "ended" : "reached 80%";
            const videoIdStr = currentPlaylistVideoId || `index-${currentPlaylistVideoIndex !== undefined ? currentPlaylistVideoIndex + 1 : 'unknown'}`;
            console.log(`[Playlist Progress] Video ${statusText}: ${videoIdStr} (index: ${currentPlaylistVideoIndex !== undefined ? currentPlaylistVideoIndex + 1 : 'unknown'}, progress: ${percent.toFixed(1)}%)`);
              ...
              try {
                ...
                if (notes) {
                  ...
                  if (metadata.videoId === currentPlaylistVideoId) {
                    ...
                    console.log(`[Playlist Progress] ✅ Found matching module: '${module.title}' for video ${currentPlaylistVideoId}`);
                    break;
                  }
                }
              } catch (e) {
            ...
            if (!matchingModule && currentPlaylistVideoIndex !== undefined && currentPlaylistVideoIndex >= 0 && currentPlaylistVideoIndex < modules.length) {
              matchingModule = modules[currentPlaylistVideoIndex];
              console.log(`[Playlist Progress] ✅ Using module by index ${currentPlaylistVideoIndex + 1}: '${matchingModule.title}'`);
            } else if (!matchingModule && !currentPlaylistVideoId && currentPlaylistVideoIndex === undefined) {
              ...
              if (videoEnded) {
                ...
                if (firstIncomplete) {
                  ...
                  console.log(`[Playlist Progress] ⚠️ No videoId/index available, using first incomplete module: '${firstIncomplete.title}'`);
                }
              }
            }
            ...
            if (matchingModule) {
              ...
              if (manuallyUncheckedModulesRef.current.has(matchingModule.id)) {
                console.log(`[Playlist Progress] ⚠️ Skipping auto-complete for '${matchingModule.title}' - manually unchecked`);
              } else if (!matchingModule.isCompleted) {
                // Skip if we just processed this module (prevent rapid toggling)
                const lastProcessed = completedModulesThisSessionRef.current.get(matchingModule.id);
                const timeSinceLastProcess = lastProcessed ? Date.now() - lastProcessed : Infinity;
                
                // Only process if enough time has passed (at least 1 second)
                if (timeSinceLastProcess >= 1000) {
                  // Mark as processed
                  completedModulesThisSessionRef.current.set(matchingModule.id, Date.now());
                  
                  try {
                    console.log(`[Playlist Progress] ✅ Marking module '${matchingModule.title}' as complete (playlist video ${currentPlaylistVideoId} ${statusText})`);
                    
                    // Use set-completion endpoint for persistence
                    await courseApi.setModuleCompletion(matchingModule.id, true);
                    
                    // Update local state immediately
                    setCourse((prev) => {
                      if (!prev) return prev;
                      const updatedModules = prev.modules.map(m =>
                        m.id === matchingModule.id ? { ...m, isCompleted: true } : m
                      );
                    ...
                    // Remove from processed set on error so we can retry
                    completedModulesThisSessionRef.current.delete(matchingModule.id);
                    console.error(`❌ Failed to mark playlist module '${matchingModule.title}' as complete:`, err);
                  }
                }
              } else {
                console.log(`[Playlist Progress] Module '${matchingModule.title}' already completed`);
              }
            } else {
              const videoIdStr = currentPlaylistVideoId || `index-${currentPlaylistVideoIndex !== undefined ? currentPlaylistVideoIndex + 1 : 'unknown'}`;
              console.log(`[Playlist Progress] ⚠️ No matching module found for playlist video ${videoIdStr} (index: ${currentPlaylistVideoIndex})`);
            }
          }
        }

        // Mark submodule complete when ~85% of its time range is watched (for non-YouTube videos)
        if (currentSubModule && !currentSubModule.isCompleted && currentSubModuleIndex >= 0 && playback.kind !== "youtube") {
          const item = subModuleTimeMap[currentSubModuleIndex];
          if (item.duration > 0) {
            const subModuleProgress = ((currentTime - item.startTime) / item.duration) * 100;
            if (subModuleProgress >= 85) {
              try {
                const subModules = (item.module.subModules || []).sort((a, b) => (a.order || 0) - (b.order || 0));
                const subModuleIndex = subModules.findIndex(sm => sm.id === currentSubModule.id);
                if (subModuleIndex >= 0) {
                  await courseApi.updateChapterProgress(course.id, currentSubModule.title, subModuleIndex, 100);
                  console.log(`[Progress] SubModule '${currentSubModule.title}' marked complete (${subModuleProgress.toFixed(1)}%)`);
                    ...
                    const updatedModules = prev.modules.map(m => {
                      if (m.id === item.module.id) {
                        const updatedSubModules = (m.subModules || []).map(sm =>
                          sm.id === currentSubModule.id ? { ...sm, isCompleted: true } : sm
                        );
                        
                        // Check if all submodules are now complete
                        const allSubModulesComplete = updatedSubModules.length > 0 && updatedSubModules.every(sm => sm.isCompleted);
                        
                        // If all submodules complete, mark module complete and update backend
                        if (allSubModulesComplete && !m.isCompleted) {
                          courseApi.toggleModuleCompletion(m.id).then(() => {
                            console.log(`[Progress] ✅ Module '${m.title}' marked complete (all submodules done)`);
                            fetchCourse({ silent: true });
                          }).catch(err => {
                            console.warn(`Failed to mark module ${m.id} as complete:`, err);
                          });
                ...
                console.warn(`Failed to mark submodule ${currentSubModule.id}:`, err);
              }
            }
          }
        }
        ...
        for (const module of modules) {
          if (!module.isCompleted) {
            const subModules = (module.subModules || []).sort((a, b) => (a.order || 0) - (b.order || 0));
            if (subModules.length > 0 && subModules.every(sm => sm.isCompleted)) {
              try {
                await courseApi.toggleModuleCompletion(module.id);
                console.log(`[Progress] ✅ Module '${module.title}' marked complete (all submodules done)`);
                ...
                console.warn(`Failed to mark module ${module.id} as complete:`, err);
              }
            }
          }
        }
        ...
        if (playback.playlistId && playlistVideoId) {
          ...
          if (currentPlaylistModule && !currentPlaylistModule.isCompleted) {
            ...
            const shouldComplete = videoEnded || (duration > 0 && percent >= 80);
            
            if (shouldComplete) {
              try {
                await courseApi.toggleModuleCompletion(currentPlaylistModule.id);
                console.log(`[Playlist Progress] ✅ Module '${currentPlaylistModule.title}' marked complete (video: ${playlistVideoId}, ${videoEnded ? 'ended' : `${percent.toFixed(1)}% watched`})`);
                
                // Update local state and progress
                setCourse((prev) => {
                  if (!prev) return prev;
                  const updatedModules = prev.modules.map(m =>
                    m.id === currentPlaylistModule.id ? { ...m, isCompleted: true } : m
                  );
                ...
                // Trigger scheduler update
                try {
                  await maybeAdjustSchedule("playlist_module_completed", currentTime, duration);
                } catch (scheduleErr) {
                  console.warn("Failed to update schedule on playlist module completion:", scheduleErr);
                }
              } catch (err) {
                console.warn(`Failed to mark playlist module ${currentPlaylistModule.id}:`, err);
              }
            }
          }
        }
        ...
        if (playback.playlistId && videoEnded && playlistVideoIndex !== undefined) {
          const sortedModules = modules.sort((a, b) => (a.order || 0) - (b.order || 0));
          if (playlistVideoIndex >= 0 && playlistVideoIndex < sortedModules.length) {
            const module = sortedModules[playlistVideoIndex];
            if (module && !module.isCompleted) {
              try {
                await courseApi.toggleModuleCompletion(module.id);
                console.log(`[Playlist] Module '${module.title}' marked complete (video ${playlistVideoIndex + 1} ended)`);
                ...
                // Trigger scheduler update
                try {
                  await maybeAdjustSchedule("playlist_video_ended", currentTime, duration);
                } catch (scheduleErr) {
                  console.warn("Failed to update schedule:", scheduleErr);
                }
              } catch (err) {
    ...
    [course, fetchCourse, maybeAdjustSchedule, playbackStats.duration, debouncedBackendSync]
  );
  ...
  const isYouTubeCourse = useCallback(() => {
    if (!course?.externalLinks || course.externalLinks.length === 0) return false;
    return course.externalLinks.some(link => 
    ...
    try {
      ...
      if (!targetCompleted) {
        moduleIds.forEach(id => {
          ...
          console.log(`[Manual Override] Module ${id} manually unchecked - will prevent auto-tracking`);
        });
      } else {
        ...
        moduleIds.forEach(id => {
          ...
          console.log(`[Manual Override] Module ${id} manually checked - allowing auto-tracking`);
        });
      }

      // Check if this is an external course (non-YouTube)
      const isExternal = isExternalCourse();

      // For external courses: If trying to mark as complete, always show quiz modal
      if (isExternal && targetCompleted && moduleIds.length > 0) {
        const moduleId = moduleIds[0]; // Get first module ID
        
        // Validate moduleId exists and is a valid number
        if (moduleId === null || moduleId === undefined || moduleId === 'undefined') {
          console.error("[Quiz] Invalid moduleId:", moduleId, "moduleIds:", moduleIds);
          setError("Invalid module ID. Please try again.");
          return;
        }
        
        // Ensure moduleId is a number
        const numericModuleId = typeof moduleId === 'number' ? moduleId : parseInt(moduleId, 10);
        if (isNaN(numericModuleId)) {
          console.error("[Quiz] moduleId is not a valid number:", moduleId);
          setError("Invalid module ID. Please try again.");
          return;
        }
        
        // Get the original state BEFORE optimistic update
        const originalCourse = course;
        const originalModule = originalCourse?.modules
          ?.flatMap(m => [m, ...(m.subModules || [])])
          .find(m => m.id === numericModuleId || m.id === moduleId);
        
        if (!originalModule) {
          console.error("[Quiz] Module not found for moduleId:", numericModuleId);
          setError("Module not found. Please refresh and try again.");
          return;
        }
        
        // Always show quiz modal for external courses when trying to complete
        // This allows users to retake quizzes even after resetting
        console.log("[Quiz] Opening quiz modal for module:", numericModuleId, originalModule.title);
        setQuizModal({
          isOpen: true,
        ...
        // Revert optimistic update - quiz modal will handle completion after quiz is passed
        setCourse((prev) => {
          if (!prev) return prev;
          const ids = new Set(moduleIds);
          const updatedModules = (prev.modules || []).map((mod) => {
            ...
            const updatedSubs = (mod.subModules || []).map((sm) => ({
              ...sm,
              isCompleted: ids.has(sm.id) || topHit ? false : sm.isCompleted, // Revert to false
            }));
            return {
              ...mod,
              isCompleted: topHit ? false : mod.isCompleted, // Revert to false
              subModules: updatedSubs,
            };
          });
          return { ...prev, modules: updatedModules };
        ...
        return; // Don't proceed with toggle - quiz modal will handle completion
      }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        <main className={styles.main}>
          <section className={styles.section}>
            ...
            {!sourceLink && (
              <p className={styles.subtle}>No source URL added yet.</p>
            )}
            ...
            {sourceLink && (
              <div className={styles.playerShell}>
                ...
                {(() => {
                  const isExternalCourse = course?.externalLinks && 
                    course.externalLinks.length > 0 &&
                    !course.externalLinks.some(link => 
                      link.platform?.toLowerCase().includes("youtube") ||
                      link.url?.includes("youtube.com") ||
                      link.url?.includes("youtu.be")
                    );

                  // Hide video playback for external courses
                  if (isExternalCourse) {
                    return (
                      <div className={styles.externalCourseInfo}>
                        <h3>External Course</h3>
                        <p>
                          This course uses external learning resources. Complete the modules and pass the quizzes to mark them as complete.
                        </p>
                        {sourceLink && (
                          <Button
                            variant="primary"
                  ...
                  // Show video playback for YouTube and other video courses
                  return (
                    <>
                      <div className={styles.viewModeToggle}>
                        <Button
                          variant={viewMode === "embedded" ? "primary" : "ghost"}
                        ...
                        <Button
                          variant={viewMode === "external" ? "primary" : "ghost"}
                          onClick={() => {
                            setViewMode("external");
                            // Track external link opening
                            if (course?.id) {
                              try {
                                const key = `learnit_external_${course.id}`;
                          ...
                          size="small"
                        >
                          Watch on {sourceLink.platform || "External Site"}
                        </Button>
                      </div>
                  ...
                  <>
                    ...
                    {playback.kind === "youtube" && (
                      <YouTubePlayer
                        key={`yt-${playback.videoId || playback.playlistId || 'default'}`}
                        playback={playback}
                        courseId={course.id}
                        onProgress={syncPlayback}
                        course={course}
                      ...
                      <div className={styles.webWrapper}>
                        <div className={styles.webToolbar}>
                          <span className={styles.subtle}>
                            Some sites block embedding. If blank, open in a new tab.
                          </span>
                          <Button
                            variant="ghost"
                            onClick={() => window.open(playback.src, "_blank")}
                          >
                            Open source
                          </Button>
                        </div>
                  ...
                  <div className={styles.externalView}>
                    <div className={styles.externalInfo}>
                      <h3>Watching on {sourceLink.platform || "External Site"}</h3>
                      <p>
                        Open the video in a new tab. When you're done, come back here to update your progress.
                      </p>
                      <div className={styles.externalActions}>
                        <Button
                          ...
                          onClick={() => {
                            ...
                            // Add tracking parameter if YouTube
                            const trackingUrl = url.includes("youtube.com") || url.includes("youtu.be")
                              ? `${url}${url.includes("?") ? "&" : "?"}t=${Math.floor(playbackStats.current || 0)}`
                              : url;
                            ...
                            if (course?.id) {
                              try {
                                ...
                                localStorage.setItem(key, JSON.stringify({
                                  ...
                                  openedAt: Date.now(),
                                  platform: sourceLink.platform,
                                  startTime: playbackStats.current || 0
                                }));
                              } catch (e) {
                                // Ignore storage errors
                              }
                        ...
                        >
                          <FaPlay /> Open in New Tab
                        </Button>
                        <Button
                          variant="ghost"
                          onClick={() => setShowManualProgress(!showManualProgress)}
                        >
                      ...
                      {showManualProgress && (
                        <div className={styles.manualProgress}>
                          <h4>Update Your Progress</h4>
                          <p>Enter how much you watched:</p>
                          <div className={styles.progressInput}>
                            <input
                              type="number"
                              min="0"
                              max="1000"
                              value={manualProgress.hours}
                              onChange={(e) => setManualProgress({
                                ...manualProgress,
                          ...
                          <div className={styles.progressActions}>
                            <Button
                              variant="primary"
                              onClick={async () => {
                                const totalHours = manualProgress.hours + (manualProgress.minutes / 60);
                                if (totalHours > 0) {
                                  try {
                                    await courseApi.updateCourseActiveTime(course.id, totalHours);
                                    await fetchCourse({ silent: true });
                                    setShowManualProgress(false);
                                    setManualProgress({ hours: 0, minutes: 0 });
                                  } catch (err) {
                                    setError(err.message || "Failed to update progress");
                                  }
                                }
                            ...
                            <Button
                              variant="secondary"
                              onClick={async () => {
                                // Mark entire course as watched
                                try {
                                  const totalHours = course.totalEstimatedHours || 10;
                                  await courseApi.updateCourseActiveTime(course.id, totalHours);
                                  // Mark all modules as complete
                                  const allModuleIds = course.modules?.flatMap(m => [
                                    m.id,
                                    ...(m.subModules || []).map(sm => sm.id)
                                  ]) || [];
                                  if (allModuleIds.length > 0) {
                                    await handleToggleModules(allModuleIds, true);
                                  }
                                  await fetchCourse({ silent: true });
                                  setShowManualProgress(false);
                                } catch (err) {
                      ...
                      <div className={styles.quickActions}>
                        <Button
                          ...
                          size="small"
                          onClick={async () => {
                            // Mark current module/submodule as complete
                            const allSubModules = course.modules
                              ?.flatMap(m => m.subModules || [])
                              .filter(sm => !sm.isCompleted) || [];
                            if (allSubModules.length > 0) {
                              const firstIncomplete = allSubModules[0];
                              try {
                                ...
                                console.warn("Failed to mark as complete:", err);
                              }
                            }
                          }}
                        >
                ...
                {playback.kind === "none" && (
                  <p className={styles.subtle}>
                    Add a course source URL to view content here.
                  </p>
                )}
                    </>
                  );
      ...
      <QuizModal
        ...
        onClose={() => setQuizModal({ isOpen: false, moduleId: null, moduleTitle: "" })}
        onQuizPassed={async () => {
          // Mark the module as complete after quiz is passed
          if (quizModal.moduleId) {
            try {
              console.log(`[Quiz] Quiz passed for module ${quizModal.moduleId}, marking as complete`);
              ...
              setCourse((prev) => {
                ...
                const updatedModules = prev.modules.map(m => {
                  ...
                  const updatedSubs = (m.subModules || []).map(sm =>
                    sm.id === quizModal.moduleId ? { ...sm, isCompleted: true } : sm
                  );
              ...
              console.error(`[Quiz] Failed to mark module ${quizModal.moduleId} as complete:`, err);
              setError(err.message || "Failed to mark module as complete. Please try again.");
            }
          }
        }}
      />
...
function InlineVideo({ src, courseId, onProgress, course }) {
  ...
  const pushProgress = useCallback(
    (reason = "timeupdate") => {
      ...
      const shouldSave = reason === "play" || reason === "paused" || reason === "seeked" || reason === "ended" || reason === "timeupdate";
      ...
      if (courseId && Number.isFinite(currentTime)) {
        try {
          const sourceLink = pickSourceLink(course?.externalLinks);
          const playback = detectPlayback(sourceLink?.url);
          const storageKey = getPlaybackStorageKey(courseId, playback.videoId, playback.playlistId);
          localStorage.setItem(
            storageKey,
            JSON.stringify({ currentTime, duration: duration || 0, timestamp: Date.now() })
          );
        } catch (e) {
  ...
  const handlePlay = () => {
    ...
    if (courseId && !activeStudySessionRef.current) {
      try {
        ...
        const currentModuleId = firstIncomplete?.id || (sortedModules.length > 0 ? sortedModules[0].id : null);
        
        console.log(`[Streak] Attempting to create study session for course ${courseId}, module ${currentModuleId || 'none'}`);
        courseApi.startStudySession(courseId, currentModuleId)
          .then(session => {
            if (session && session.id) {
              activeStudySessionRef.current = session.id;
              studySessionStartTimeRef.current = Date.now();
              console.log(`[Streak] ✅ Auto-created study session ${session.id} for course ${courseId} at ${new Date().toLocaleTimeString()}`);
            } else {
              console.warn(`[Streak] Session created but no ID returned:`, session);
            }
          })
          .catch(async err => {
            // If session already exists, try to get the active session
            if (err?.response?.status === 400 || err?.message?.includes('already exists')) {
              console.log(`[Streak] Session already exists for course ${courseId} - fetching existing session...`);
              try {
                const sessions = await courseApi.getCourseSessions(courseId);
                const activeSession = Array.isArray(sessions) 
                  ? sessions.find(s => {
                      const isCompleted = s.isCompleted === true || s.IsCompleted === true;
                ...
                if (activeSession) {
                  ...
                  const startTimeStr = activeSession.startTime || activeSession.StartTime;
                  ...
                  if (sessionId) {
                    ...
                    console.log(`[Streak] ✅ Using existing active session ${sessionId} (started ${duration.toFixed(1)} minutes ago)`);
                  }
                }
              } catch (fetchErr) {
                console.warn(`[Streak] Failed to get existing sessions:`, fetchErr);
              }
            } else {
  ...
  const handlePause = () => {
    ...
    if (activeStudySessionRef.current && courseId) {
      ...
      const watchedDuration = startTime ? (Date.now() - startTime) / 1000 / 60 : 0; // minutes
      const MIN_SESSION_MINUTES = 1; // Minimum 1 minute to count toward streak
      
      if (watchedDuration >= MIN_SESSION_MINUTES) {
        console.log(`[Streak] Video paused after ${watchedDuration.toFixed(1)} minutes - completing session ${sessionId}`);
        courseApi.stopStudySession(sessionId, `Auto-completed: Video paused (watched ${watchedDuration.toFixed(1)} minutes)`)
          .then(() => {
            console.log(`[Streak] ✅ Auto-completed study session ${sessionId} on pause`);
            activeStudySessionRef.current = null;
            studySessionStartTimeRef.current = null;
          })
          .catch(err => {
            console.warn(`[Streak] Failed to complete study session on pause:`, err);
          });
      } else {
        console.log(`[Streak] Video paused but session too short (${watchedDuration.toFixed(1)} min < ${MIN_SESSION_MINUTES} min) - keeping session active`);
      }
    }
  };
  ...
  const handleEnded = () => {
    ...
    if (activeStudySessionRef.current && courseId) {
      ...
      if (watchedDuration >= MIN_SESSION_MINUTES) {
        courseApi.stopStudySession(sessionId, "Auto-completed: Video ended")
          .then(() => {
            console.log(`[Streak] ✅ Auto-completed study session ${sessionId} (watched ${watchedDuration.toFixed(1)} minutes)`);
            activeStudySessionRef.current = null;
            studySessionStartTimeRef.current = null;
          })
          .catch(err => {
        ...
        console.log(`[Streak] Session ${sessionId} too short (${watchedDuration.toFixed(1)} min < ${MIN_SESSION_MINUTES} min), completing anyway`);
        courseApi.stopStudySession(sessionId, "Auto-completed: Video ended (short session)")
          .then(() => {
            activeStudySessionRef.current = null;
            studySessionStartTimeRef.current = null;
          })
    ...
    () => {
      ...
      if (courseId && el) {
        const restorePosition = () => {
          try {
            const sourceLink = pickSourceLink(course?.externalLinks);
            const playback = detectPlayback(sourceLink?.url);
            const storageKey = `learnit_playback_${courseId}_${playback.videoId || playback.playlistId || 'default'}`;
            const raw = localStorage.getItem(storageKey);
            if (raw) {
              const saved = JSON.parse(raw);
              if (Number.isFinite(saved.currentTime) && saved.currentTime > 0 && saved.duration > 0) {
      ...
      const handleVisibilityChange = () => {
        if (document.hidden && activeStudySessionRef.current && courseId) {
          ...
          if (watchedDuration >= MIN_SESSION_MINUTES) {
            courseApi.stopStudySession(sessionId, `Auto-completed: Tab hidden (watched ${watchedDuration.toFixed(1)} minutes)`)
              .then(() => {
                console.log(`[Streak] ✅ Auto-completed study session ${sessionId} on tab hidden`);
                activeStudySessionRef.current = null;
                studySessionStartTimeRef.current = null;
              })
              .catch(err => {
                console.warn(`[Streak] Failed to complete study session on tab hidden:`, err);
                activeStudySessionRef.current = null;
                studySessionStartTimeRef.current = null;
              });
          }
      ...
      const handleBeforeUnload = () => {
        if (activeStudySessionRef.current && courseId) {
          ...
          if (watchedDuration >= MIN_SESSION_MINUTES) {
            const notes = `Auto-completed: Page unload (watched ${watchedDuration.toFixed(1)} minutes)`;
      ...
      return () => {
        ...
        if (activeStudySessionRef.current && courseId) {
          ...
          if (watchedDuration >= MIN_SESSION_MINUTES) {
            courseApi.stopStudySession(sessionId, "Auto-completed: User navigated away")
              .then(() => {
                console.log(`[Streak] ✅ Auto-completed study session ${sessionId} on cleanup (watched ${watchedDuration.toFixed(1)} minutes)`);
              })
              .catch(err => {
                console.warn(`[Streak] Failed to complete study session on cleanup:`, err);
              });
          } else {
            // Session too short - complete anyway
            courseApi.stopStudySession(sessionId, "Auto-completed: User navigated away (short session)")
              .catch(err => {
                console.warn(`[Streak] Failed to complete short session on cleanup:`, err);
              });
          }
...
function YouTubePlayer({ playback, courseId, onProgress, course }) {
  ...
  const savePositionOnLeave = useCallback(async () => {
    ...
    try {
      ...
      if (currentTime >= 2 && Number.isFinite(currentTime) && Number.isFinite(duration) && duration > 0) {
        console.log(`[YouTube] Saving position on page leave: ${currentTime.toFixed(1)}s / ${duration.toFixed(1)}s`);
        ...
        try {
          ...
          if (playback.playlistId && saveVideoId) {
            try {
              ...
              console.warn("Failed to save last video to localStorage on leave:", e);
            }
          }
          
          console.log(`[YouTube] ✓ Position saved on leave: ${currentTime.toFixed(1)}s (videoId: ${saveVideoId || 'NONE'})`);
        } catch (err) {
  ...
  useEffect(() => {
    ...
    if (playerRef.current || !containerRef.current) {
      console.log("[YouTube Player] Player already exists or container missing, skipping creation");
      return;
    }
    ...
    const setup = async () => {
      ...
      if (playerRef.current) {
        console.log("[YouTube Player] Player was created during async setup, skipping");
        return;
      }
      ...
      if (courseId && playback.videoId) {
        try {
          ...
          try {
            const dbPosition = await courseApi.getPlaybackPosition(courseId, playback.videoId, null);
            if (dbPosition && Number.isFinite(dbPosition.currentTimeSeconds) && dbPosition.currentTimeSeconds >= 2) {
              savedStartTime = Math.floor(dbPosition.currentTimeSeconds);
              console.log(`[YouTube] Found database position for start parameter: ${savedStartTime}s`);
            }
          } catch (e) {
          ...
          if (savedStartTime === 0) {
            ...
            if (raw) {
              try {
                const saved = JSON.parse(raw);
                if (Number.isFinite(saved.currentTime) && saved.currentTime >= 2) {
                  savedStartTime = Math.floor(saved.currentTime);
                  console.log(`[YouTube] Found localStorage position for start parameter: ${savedStartTime}s`);
                }
              } catch (e) {
                // Ignore
              }
      ...
      if (playback.playlistId) {
            ...
            try {
              ...
              if (lastVideo.videoId && lastVideo.timestamp) {
                ...
                if (age < 30 * 24 * 60 * 60 * 1000) {
                  ...
                  console.log(`[YouTube Playlist] Found last watched video in localStorage: ${videoId}`);
                  
                  // Try to get the saved position for this video
                  try {
                    const savedPos = await courseApi.getPlaybackPosition(courseId, videoId, null);
                    if (savedPos && Number.isFinite(savedPos.currentTimeSeconds) && savedPos.currentTimeSeconds >= 2) {
                      resumeTime = Math.floor(savedPos.currentTimeSeconds);
                      console.log(`[YouTube Playlist] Resuming from saved position: ${videoId} at ${resumeTime}s`);
                    }
                  } catch (e) {
                    ...
                    if (raw) {
                      try {
                        ...
                        if (Number.isFinite(saved.currentTime) && saved.currentTime >= 2) {
                          ...
                          console.log(`[YouTube Playlist] Resuming from localStorage position: ${videoId} at ${resumeTime}s`);
                        }
                      } catch (e2) {
                        // Ignore
                      }
              ...
              console.warn("[YouTube Playlist] Failed to parse last video data:", e);
            }
          }
        } catch (e) {
          console.warn("[YouTube Playlist] Failed to get last video from localStorage:", e);
        }
        ...
        if (!videoId) {
          ...
          const firstIncompleteModule = sortedModules.find(m => !m.isCompleted);
          
          if (firstIncompleteModule) {
            try {
              // Extract videoId from module's Notes field
              const notes = firstIncompleteModule.notes || firstIncompleteModule.Notes || "";
              if (notes) {
                const metadata = JSON.parse(notes);
                if (metadata.videoId) {
                  videoId = metadata.videoId;
                  console.log(`[YouTube Playlist] Resuming from first incomplete module: '${firstIncompleteModule.title}' (video: ${videoId})`);
                } else {
                  console.log(`[YouTube Playlist] Module '${firstIncompleteModule.title}' has no videoId in Notes, starting from beginning`);
                }
              } else {
                // If no Notes, try to use module index to estimate which video
                const moduleIndex = sortedModules.indexOf(firstIncompleteModule);
                console.log(`[YouTube Playlist] Module '${firstIncompleteModule.title}' (index ${moduleIndex}) has no Notes, starting from beginning`);
              }
            } catch (e) {
              console.warn(`[YouTube Playlist] Failed to parse Notes for module '${firstIncompleteModule.title}':`, e);
            }
          } else {
            console.log("[YouTube Playlist] All modules completed, starting from beginning");
          }
        }
        ...
        if (videoId) {
          console.log(`[YouTube Player] Loading playlist from video: ${videoId}${resumeTime > 0 ? ` at ${resumeTime}s` : ''}`);
        } else {
          console.log("[YouTube Player] Loading playlist from beginning (no resume data found)");
        }
      ...
      if (resumeTime > 0) {
        ...
        console.log(`[YouTube Resume] ✓✓✓ Setting start parameter to ${resumeTime}s (${(resumeTime/60).toFixed(1)} minutes) - video will start here automatically`);
        ...
        console.log(`[YouTube Resume] No saved start time found (resumeTime=${resumeTime}) - will start from beginning`);
      ...
      if (!playback.playlistId && videoId) {
        ...
        if (videoId) {
          playerConfig.videoId = videoId;
          console.log(`[YouTube Player] Playlist will start at video: ${videoId}${resumeTime > 0 ? ` at ${resumeTime}s` : ''}`);
        } else {
      ...
      try {
        ...
        if (playerRef.current) {
          console.log("[YouTube Player] Player already exists, aborting creation");
          return;
        }
        ...
        playerRef.current = new YT.Player(containerRef.current, {
          ...
          onReady: async (event) => {
            ...
            if (!courseId || cancelled) {
              console.log(`[YouTube Resume] onReady: Skipping - courseId=${courseId}, cancelled=${cancelled}`);
              isResumingRef.current = false;
              return;
            }
            ...
            if (hasResumedRef.current) {
              console.log(`[YouTube Resume] onReady: Already resumed via start parameter (start=${playerVars.start || 'none'}), clearing resume flag in 3s`);
              ...
              setTimeout(() => {
                isResumingRef.current = false;
                console.log(`[YouTube Resume] Resume complete (start parameter) - module checks enabled`);
              }, 3000);
              return;
            }
            
            console.log(`[YouTube Resume] onReady: Starting resume lookup...`);
            isResumingRef.current = true;
            ...
            try {
              ...
              if (playback.playlistId && !actualVideoId && event.target && typeof event.target.getVideoData === 'function') {
                try {
                  const videoData = event.target.getVideoData();
                  if (videoData && videoData.video_id) {
                    actualVideoId = videoData.video_id;
                    console.log(`[YouTube] Got playlist video ID from player: ${actualVideoId}`);
                  }
                } catch (e) {
              ...
              else if (!playback.playlistId && !actualVideoId && event.target && typeof event.target.getVideoData === 'function') {
                try {
                  const videoData = event.target.getVideoData();
                  if (videoData && videoData.video_id) {
                    actualVideoId = videoData.video_id;
                    console.log(`[YouTube] Got video ID from player (fallback): ${actualVideoId}`);
                  }
                } catch (e) {
              ...
              console.log(`[YouTube] Resume lookup - videoId: ${actualVideoId || 'NONE'}, playlistId: ${playback.playlistId || 'NONE'}, playback.videoId: ${playback.videoId || 'NONE'}`);
              ...
              try {
                ...
                const lookupVideoId = actualVideoId || playback.playlistId || "";
                if (lookupVideoId && courseId) {
                        ...
                        try {
                          ...
                          if (notes) {
                            ...
                            if (metadata.videoId === actualVideoId) {
                              moduleId = module.id;
                              console.log(`[YouTube] Found matching module ${moduleId} for video ${actualVideoId}`);
                              break;
                            }
                          }
                        } catch (e) {
                  ...
                  console.log(`[YouTube Resume] Looking up playback position: courseId=${courseId}, videoId=${lookupVideoId}, moduleId=${moduleId || 'null'}, actualVideoId=${actualVideoId || 'NONE'}, playback.videoId=${playback.videoId || 'NONE'}`);
                  ...
                  if (course?.modules) {
                    ...
                    for (let idx = sortedModules.length - 1; idx >= 0; idx--) {
                      ...
                      if (mod.isCompleted) {
                        try {
                          ...
                          if (notes) {
                            const metadata = JSON.parse(notes);
                            if (metadata.startTimeSeconds !== undefined && metadata.startTimeSeconds > 0) {
                              lastCompletedModule = mod;
                              lastCompletedIndex = idx;
                              smartResumeTime = metadata.startTimeSeconds;
                              console.log(`[YouTube Resume] 🎯 Found last completed module: '${mod.title}' at ${smartResumeTime.toFixed(1)}s (index ${idx})`);
                              break;
                            }
                          }
                        } catch (e) {
                    ...
                    if (lastCompletedIndex >= 0 && lastCompletedIndex < sortedModules.length - 1) {
                      const nextModule = sortedModules[lastCompletedIndex + 1];
                      try {
                        const notes = nextModule.notes || nextModule.Notes || "";
                        if (notes) {
                          const metadata = JSON.parse(notes);
                          if (metadata.startTimeSeconds !== undefined && metadata.startTimeSeconds > 0) {
                            smartResumeTime = metadata.startTimeSeconds;
                            console.log(`[YouTube Resume] 🎯 Resuming from next module: '${nextModule.title}' at ${smartResumeTime.toFixed(1)}s (after completed '${lastCompletedModule.title}')`);
                          }
                        }
                      } catch (e) {
                        // If next module doesn't have startTime, use last completed module's start time
                        console.log(`[YouTube Resume] Next module has no startTime, using last completed module's start: ${smartResumeTime.toFixed(1)}s`);
                      }
                    }
                  }
                  
                  // PRIORITY 1: Try to get actual saved playback position from database (where user left off)
                  const dbPosition = await courseApi.getPlaybackPosition(courseId, lookupVideoId, moduleId);
                  ...
                  if (dbPosition && Number.isFinite(dbPosition.currentTimeSeconds) && dbPosition.currentTimeSeconds >= 2) {
                    ...
                    console.log(`[YouTube Resume] ✓✓✓ Loaded actual saved position from database: ${savedTime.toFixed(1)}s / ${savedDuration.toFixed(1)}s (${(savedTime/60).toFixed(1)} minutes)`);
                  } else if (dbPosition && dbPosition.currentTimeSeconds > 0 && dbPosition.currentTimeSeconds < 2) {
                    console.log(`[YouTube Resume] Database has position ${dbPosition.currentTimeSeconds.toFixed(1)}s but it's too small (< 2s), trying fallback...`);
                    // Try without moduleId as fallback
                    if (moduleId) {
                      const dbPositionNoModule = await courseApi.getPlaybackPosition(courseId, lookupVideoId, null);
                      if (dbPositionNoModule && Number.isFinite(dbPositionNoModule.currentTimeSeconds) && dbPositionNoModule.currentTimeSeconds >= 2) {
                        savedTime = dbPositionNoModule.currentTimeSeconds;
                        savedDuration = dbPositionNoModule.durationSeconds || 0;
                        console.log(`[YouTube Resume] ✓✓✓ Loaded from database (no moduleId): ${savedTime.toFixed(1)}s / ${savedDuration.toFixed(1)}s (${(savedTime/60).toFixed(1)} minutes)`);
                      }
                    }
                  } else {
                    console.log(`[YouTube Resume] No database position found for videoId=${lookupVideoId}, trying smart resume...`);
                  }
                  ...
                  if (savedTime < 2 && smartResumeTime >= 2) {
                    ...
                    savedDuration = dbPosition?.durationSeconds || 0;
                    console.log(`[YouTube Resume] 🎯 Using smart resume from last completed module (no saved position): ${savedTime.toFixed(1)}s (${(savedTime/60).toFixed(1)} minutes)`);
                  }
                }
              } catch (err) {
                console.warn("Failed to load playback position from database:", err);
              }
              ...
              if (savedTime < 2) {
                for (const storageKey of storageKeys) {
                  if (!storageKey.includes('_') || storageKey.endsWith('_')) continue;
                  ...
                  if (raw) {
                    try {
                      ...
                      if (Number.isFinite(saved.currentTime) && saved.currentTime >= 1) {
                        ...
                        if (saved.currentTime > savedTime) {
                          ...
                          console.log(`[YouTube] ✓ Loaded from localStorage (${storageKey}): ${savedTime.toFixed(1)}s / ${savedDuration.toFixed(1)}s`);
                          break;
                        }
                      }
                    } catch (e) {
              ...
              if (savedTime >= 2) {
                console.log(`[YouTube Resume] Attempting to resume from ${savedTime.toFixed(1)}s (${(savedTime/60).toFixed(1)} minutes)...`);
                ...
                const seekToPosition = () => {
                  if (cancelled || !event.target) {
                    console.log(`[YouTube] Resume cancelled or player not available`);
                    return;
                  }
                  ...
                  try {
                    ...
                    try {
                      ...
                      if (e.name === 'SecurityError') {
                        ...
                        currentTime = 0;
                        playerState = -1;
                        console.log(`[YouTube] SecurityError getting player state, using saved duration: ${duration.toFixed(1)}s`);
                      } else {
                        throw e;
                      }
                    }
                    
                    console.log(`[YouTube] Resume attempt ${retryCount + 1}/${maxRetries}: duration=${duration.toFixed(1)}s, current=${currentTime.toFixed(1)}s, state=${playerState}`);
                    
                    if (duration > 0 && duration > 1) {
                      // Use saved duration if available, otherwise use player duration
                      const maxTime = savedDuration > 0 ? Math.min(savedDuration, duration) : duration;
                      const seekTime = Math.max(0, Math.min(savedTime, maxTime - 1));
                      
                      // Only seek if the time difference is significant (more than 2 seconds)
                      const timeDiff = Math.abs(seekTime - currentTime);
                      if (timeDiff > 2) {
                        console.log(`[YouTube] Seeking from ${currentTime.toFixed(1)}s to ${seekTime.toFixed(1)}s (diff: ${timeDiff.toFixed(1)}s)`);
                        try {
                          ...
                          console.log(`[YouTube] ✓ Resumed playback at ${seekTime.toFixed(1)}s / ${duration.toFixed(1)}s`);
                          
                          // Mark resume as complete immediately - seek is done
                          isResumingRef.current = false;
                          console.log(`[YouTube] Resume complete - module checks enabled`);
                            ...
                            setTimeout(() => {
                              try {
                                if (event.target && typeof event.target.playVideo === 'function') {
                                  ...
                                  console.log(`[YouTube] Attempted to play video after seek`);
                                }
                              } catch (e) {
                                // Ignore play errors
                              }
                          ...
                          console.log(`[YouTube] Seek attempted (SecurityError ignored): ${seekTime.toFixed(1)}s`);
                        }
                      } else {
                        console.log(`[YouTube] Already at saved position (${currentTime.toFixed(1)}s ≈ ${seekTime.toFixed(1)}s, diff: ${timeDiff.toFixed(1)}s)`);
                        // Mark resume as complete if already at position
                        setTimeout(() => {
                          isResumingRef.current = false;
                          console.log(`[YouTube] Resume complete (already at position) - module checks enabled`);
                        }, 1000);
                      }
                      ...
                      console.log(`[YouTube] Player duration not ready (${duration}), retrying in ${delay}ms (attempt ${retryCount}/${maxRetries})`);
                      setTimeout(seekToPosition, delay);
                    } else {
                      console.warn(`[YouTube] Could not get valid duration after ${maxRetries} retries (got ${duration}), skipping resume`);
                      // Mark resume as complete even if failed (to prevent blocking forever)
                      isResumingRef.current = false;
                      console.log(`[YouTube] Resume failed - module checks enabled`);
                    }
                  } catch (err) {
                    // Ignore SecurityError - it's expected for cross-origin access
                    if (err.name === 'SecurityError') {
                      console.log(`[YouTube] SecurityError on resume (expected for cross-origin), attempting seek anyway: ${savedTime.toFixed(1)}s`);
                      // Try to seek anyway even with SecurityError
                      try {
                        if (event.target && typeof event.target.seekTo === 'function') {
                          const seekTime = Math.min(savedTime, savedDuration > 0 ? savedDuration - 1 : savedTime);
                          event.target.seekTo(seekTime, true);
                          console.log(`[YouTube] Seek attempted despite SecurityError: ${seekTime.toFixed(1)}s`);
                        }
                      } catch (e) {
                        // Ignore
                      }
                    ...
                    if (retryCount < maxRetries) {
                      ...
                      console.warn(`[YouTube] Seek failed, retrying in ${delay}ms (attempt ${retryCount}/${maxRetries}):`, err);
                      setTimeout(seekToPosition, delay);
                    } else {
                      console.warn("Failed to seek on resume after retries:", err);
                    }
                  }
                };
                ...
                console.log(`[YouTube Resume] No saved playback position found (savedTime=${savedTime.toFixed(1)}s, required >= 2s), starting from beginning`);
          ...
          onStateChange: (event) => {
            ...
            try {
              ...
              if (!Number.isFinite(current) || !Number.isFinite(duration) || duration <= 0) {
                console.warn("[YouTube Player] Invalid time values:", { current, duration });
                return;
              }
              ...
              const stateNames = ['UNSTARTED', 'ENDED', 'PLAYING', 'PAUSED', 'BUFFERING', 'CUED'];
              console.log(`[YouTube Player] State: ${stateNames[event.data] || event.data}, Time: ${current.toFixed(1)}s / ${duration.toFixed(1)}s (${percent.toFixed(1)}%)`);
              ...
              if (event.data === YT.PlayerState.PLAYING) {
                  ...
                  try {
                    ...
                    for (const module of modules) {
                      try {
                        ...
                        if (notes) {
                          const metadata = JSON.parse(notes);
                          if (metadata.startTimeSeconds !== undefined && current >= metadata.startTimeSeconds) {
                    ...
                    courseApi.startStudySession(courseId, currentModuleId)
                      ...
                      .catch(async err => {
                        ...
                        if (err?.response?.status === 400 || err?.message?.includes('already exists')) {
                          ...
                          try {
                            const sessions = await courseApi.getCourseSessions(courseId);
                            console.log(`[Streak] Fetched ${Array.isArray(sessions) ? sessions.length : 0} sessions for course ${courseId}`);
                            ...
                            if (activeSession) {
                              ...
                              if (sessionId) {
                                ...
                                console.warn(`[Streak] Active session found but no ID:`, activeSession);
                              }
                            } else {
                              console.warn(`[Streak] No active session found. All sessions:`, sessions);
                            }
                          } catch (fetchErr) {
                            console.warn(`[Streak] Failed to get existing sessions:`, fetchErr);
                          }
                          ...
                          console.error(`[Streak] ❌ Failed to create study session:`, err);
                          console.error(`[Streak] Error details:`, err?.response?.data || err?.message || err);
                        }
                      });
                  } catch (err) {
                    console.warn(`[Streak] Error creating study session:`, err);
                ...
                if (isResumingRef.current && current < 5 && lastSavedTimeRef.current > 5) {
                  // Video started playing but we're not at saved position yet - wait for seek
                  console.log(`[YouTube Resume] Video playing but not at saved position yet (current: ${current.toFixed(1)}s, saved: ${lastSavedTimeRef.current.toFixed(1)}s), waiting for seek...`);
                  setTimeout(() => {
                    // Check again after delay
                    try {
                      const checkTime = event.target.getCurrentTime();
                      if (Math.abs(checkTime - lastSavedTimeRef.current) < 3) {
                        // Close enough to saved position
                        isResumingRef.current = false;
                        console.log(`[YouTube Resume] Resume complete after play - module checks enabled`);
                      } else {
                        // Still not at position, but enable checks anyway after timeout
                        setTimeout(() => {
                          isResumingRef.current = false;
                          console.log(`[YouTube Resume] Resume timeout - module checks enabled`);
                        }, 2000);
                      }
                    } catch (e) {
                      ...
                      setTimeout(() => {
                        isResumingRef.current = false;
                        console.log(`[YouTube Resume] Resume timeout (error) - module checks enabled`);
                      }, 2000);
                    }
                  }, 1000);
                  ...
                  isResumingRef.current = false;
                  console.log(`[YouTube Resume] Resume complete (video past initial seconds) - module checks enabled`);
                }
                
                // Save on play
                onProgress?.({ currentTime: current, duration, percent }, "play");
                ...
                if (courseId && Number.isFinite(current) && current >= 1) {
                  ...
                  try {
                    ...
                    localStorage.setItem(
                      storageKey,
                      JSON.stringify({ currentTime: current, duration, timestamp: Date.now() })
                    );
                  } catch (e) {
                    // Ignore storage errors
                  }
                ...
                intervalRef.current = setInterval(() => {
                  ...
                  try {
                    ...
                    if (Number.isFinite(cur) && Number.isFinite(dur) && dur > 0 && cur >= 0) {
                      ...
                      if (playback.playlistId && event.target) {
                        try {
                          ...
                          if (playlistVideoId && playlistInfo && Array.isArray(playlistInfo)) {
                            playlistVideoIndex = playlistInfo.indexOf(playlistVideoId);
                          }
                        } catch (err) {
                          // SecurityError is expected for cross-origin access - ignore it
                          if (err.name !== 'SecurityError') {
                            console.warn("Failed to get playlist info during playback:", err);
                          }
                        }
                      }
                      ...
                      if (courseId && cur >= 1) {
                        try {
                          const storageKey = `learnit_playback_${courseId}_${playback.videoId || playlistVideoId || playback.playlistId || 'default'}`;
                          localStorage.setItem(
                            storageKey,
                            JSON.stringify({ currentTime: cur, duration: dur, timestamp: Date.now() })
                          );
                        } catch (e) {
                          // Ignore storage errors
                        }
                      ...
                      const timeJump = Math.abs(cur - lastSavedTimeRef.current);
                      if (timeJump > 5) {
                        // Significant time jump = seek happened
                        lastSavedTimeRef.current = cur;
                        onProgress?.(
                          { currentTime: cur, duration: dur, percent: percentFromTimes(cur, dur), playlistVideoIndex, playlistVideoId },
                          "seeked"
                        );
                      } else {
                        ...
                        if (now - lastCheckTime > 3000) {
                          ...
                          onProgress?.(
                            { currentTime: cur, duration: dur, percent, playlistVideoIndex, playlistVideoId, videoEnded: false },
                            "timeupdate"
                          );
                        }
                      }
                ...
                if (courseId && Number.isFinite(current) && (current >= 1 || (event.data === YT.PlayerState.ENDED && current > 0))) {
                  lastSavedTimeRef.current = current;
                  try {
                    const storageKey = `learnit_playback_${courseId}_${playback.videoId || playback.playlistId || 'default'}`;
                    localStorage.setItem(
                ...
                onProgress?.({ currentTime: current, duration, percent }, "paused");
                
                // AUTO-COMPLETE STUDY SESSION WHEN PAUSED (if watched for minimum duration)
                // This ensures sessions are completed even if user pauses and doesn't resume
                if (event.data === YT.PlayerState.PAUSED && activeStudySessionRef.current && courseId) {
                  const sessionId = activeStudySessionRef.current;
                  const startTime = studySessionStartTimeRef.current;
                  const watchedDuration = startTime ? (Date.now() - startTime) / 1000 / 60 : 0; // minutes
                  const MIN_SESSION_MINUTES = 1; // Minimum 1 minute to count toward streak
                ...
                if (event.data === YT.PlayerState.ENDED) {
                  ...
                  if (playback.playlistId && event.target) {
                    try {
                      ...
                      if (playlistVideoId && playlistInfo && Array.isArray(playlistInfo)) {
                        playlistVideoIndex = playlistInfo.indexOf(playlistVideoId);
                        console.log(`[Playlist] Video ${playlistVideoIndex + 1} ended: ${playlistVideoId}`);
                      }
                    } catch (err) {
                      // SecurityError is expected for cross-origin access - ignore it
                      if (err.name !== 'SecurityError') {
                        console.warn("Failed to get playlist info on video end:", err);
                      }
                    }
                  }
    ...
    return () => {
      ...
      if (activeStudySessionRef.current && courseId) {
        ...
        if (watchedDuration >= MIN_SESSION_MINUTES) {
          ...
          console.log(`[Streak] Session ${sessionId} too short on cleanup (${watchedDuration.toFixed(1)} min < ${MIN_SESSION_MINUTES} min)`);
          courseApi.stopStudySession(sessionId, "Auto-completed: User navigated away (short session)")
            .catch(err => {
              console.warn(`[Streak] Failed to complete short session on cleanup:`, err);
            });
