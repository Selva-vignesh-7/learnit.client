...
const Awards = () => {
  ...
  const loadAwards = async () => {
    try {
      ...
      if (checkResult.awards && checkResult.awards.length > 0) {
        showToast(`🎉 Congratulations! You earned ${checkResult.awards.length} new award(s)!`, 'success');
  ...
  const categories = ['all', 'hours', 'courses', 'consistency', 'longeststreak'];
      ...
      <div className={styles.awardsGrid}>
        {filteredAwards.length === 0 ? (
          ...
          filteredAwards.map(award => (
            <div
              key={award.id}
              className={`${styles.awardCard} ${award.isEarned ? styles.earned : styles.locked}`}
            >
                ...
                {award.isEarned ? (
                  <div className={styles.earnedBadge}>
                    ...
                    {award.progressValue && (
                      <span className={styles.progressValue}>
                        {award.progressValue} {award.category === 'hours' ? 'hours' : award.category === 'courses' ? 'courses' : 'days'}
                      </span>
                    )}
                  </div>
                ) : (
                  <div className={styles.progressBar}>
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, award.progressPercentage || 0)}%`,
                        backgroundColor: award.color || '#2563eb',
                      }}
                    />
                    <span className={styles.progressText}>
                      {award.progressPercentage ? `${Math.round(award.progressPercentage)}%` : '0%'}
