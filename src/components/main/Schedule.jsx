...
export default function Schedule() {
  ...
  const checkForMissedEvents = (eventList) => {
    ...
    const missed = eventList.filter((e) => {
      ...
      const endTime = e.end ? new Date(e.end) : new Date(new Date(e.start).getTime() + 60 * 60 * 1000);
      return endTime < now;
    });
  ...
  const handleAutoSchedule = async () => {
    try {
      ...
      showNotification(
        ...
        showNotification("No course modules available to schedule. All modules may already be scheduled.");
      }
    } catch (err) {
  ...
  const handleLinkToModule = async (eventId, moduleId) => {
    try {
      ...
      const errorMessage = err.message || err.data?.message || "Failed to link event to module";
  ...
  async function handleSelect(info) {
    ...
    const end = info.endStr ? new Date(info.endStr) : new Date(start.getTime() + 60 * 60 * 1000);
    ...
    if (autoOptions.dayStart && autoOptions.dayEnd && !info.allDay) {
    ...
    try {
      ...
      const errorMessage = err.message || err.data?.message || "Failed to create event";
      if (err.status === 409) {
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      ...
      const errorMessage = err.message || err.data?.message || "Failed to update event";
      if (err.status === 409) {
        setError(`Conflict: ${errorMessage}`);
      } else if (err.status === 400) {
        setError(`Validation error: ${errorMessage}`);
            ...
            <div className={styles.actionButtons}>
              {availableModules.length === 0 && courses.length > 0 ? (
                ...
                <button
                  ...
                  title={availableModules.length === 0 ? "No modules available to schedule" : ""}
                >
